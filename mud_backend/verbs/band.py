    ...
    def execute(self):
        ...
        if not found_obj:
            ...
            for detail in all_statics:
                ...
                if clean_target in d_keys or (d_name and (clean_target == d_name or clean_target == clean_name(d_name))):
                    found_obj = detail
                    break
...
class Get(BaseVerb):
    def execute(self):
        ...
        target_hand_slot = "mainhand" if not self.player.worn_items.get("mainhand") else ("offhand" if not self.player.worn_items.get("offhand") else None)
