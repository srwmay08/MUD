...
class ConnectionManager:
    ...
    def broadcast_to_room(self, room_id: str, message: str, msg_type: str, skip_sid: Optional[Union[str, List[str], Set[str]]] = None):
        ...
        logger.debug(f"Room: {room_id} | Msg: {message[:30]}... | Candidates: {players_in_room}")

        for player_name in players_in_room:
            player_info = self.world.get_player_info(player_name)
            if not player_info: 
                logger.debug(f"Skipped {player_name} (No Info)")
                continue
            ...
            if not player_obj or not sid: 
                logger.debug(f"Skipped {player_name} (No SID/Obj)")
                continue
            
            if sid in skip_sids_set:
                logger.debug(f"Skipped {player_name} (SID {sid} is in skip list)")
                continue
            ...
            logger.debug(f"Sending to {player_name} (SID: {sid})")
    ...
    def disconnect_player(self, sid: str):
        ...
        if player_to_remove:
            logger.info(f"[CONNECTION] Player {player_to_remove} disconnected.")
