...
# --- (Config, Stances, Shield, Race data are unchanged) ---
...
# ---
# --- THIS IS THE FIX: Moved calculate_roundtime() here ---
# ---
def calculate_roundtime(agility: int) -> float:
    """Calculates combat roundtime based on agility."""
...
# --- (calculate_roundtime was MOVED UP) ---

# ---
# --- MODIFIED: _find_combatant
# ---
...
def process_combat_tick(broadcast_callback, send_to_player_callback):
    ...
    for combatant_id, state in combatant_list:
        ...
        # Check if combat was stopped by a previous iteration
        # --- ADD LOCK ---
        with game_state.COMBAT_LOCK:
            if combatant_id not in game_state.COMBAT_STATE:
                continue # This combatant was removed, skip
        # --- END LOCK ---
        ...
        if not attacker or not defender or not room_id:
            stop_combat(combatant_id, state["target_id"]) # This handles its own lock
            continue
        ...
        if is_attacker_player:
            ...
            if attacker_posture != "standing":
                ...
                # --- ADD LOCK ---
                with game_state.COMBAT_LOCK:
                    if combatant_id in game_state.COMBAT_STATE: # Check if still in combat
                        game_state.COMBAT_STATE[combatant_id]["next_action_time"] = current_time + 1.0
                # --- END LOCK ---
                continue
        # --- END NEW CHECK ---
        ...
        if attack_results['hit']:
            ...
            if is_defender_player:
                ...
                # --- ADD LOCK ---
                # This block modifies RUNTIME_MONSTER_HP and DEFEATED_MONSTERS
                with game_state.COMBAT_LOCK:
                    ...
                    if new_hp <= 0:
                        ...
                        # We call stop_combat *inside* the lock, which is why we MUST use RLock
                        stop_combat(combatant_id, state["target_id"])
                        continue
                    else:
                        if is_attacker_player:
        ...
        # --- ADD LOCK ---
        with game_state.COMBAT_LOCK:
            # Check if combatant still exists before setting RT
